...
export default function FleetPanel() {
  ...
  return (
    <div className="surface-card mt-6 p-6">
      <h2 className="text-lg font-bold text-gray-800">Fleet</h2>
      <p className="mt-1 text-sm text-gray-600">Register vehicles and drivers. New entries need admin verification before they can carry bookings.</p>

      {error && <p className="mt-3 rounded-xl bg-red-50 px-4 py-3 text-sm text-red-700">{error}</p>}

      <div className="mt-5">
        <h3 className="text-sm font-bold uppercase tracking-wide text-gray-500">Vehicles</h3>
        <form onSubmit={addVehicle} className="mt-2 flex flex-wrap items-end gap-2">
          <select className="input-field w-auto" value={vehicleForm.serviceType} onChange={(e) => setVehicleForm({ ...vehicleForm, serviceType: e.target.value })}>
          ...
          />
          <input className="input-field w-auto" placeholder="Make" value={vehicleForm.make} onChange={(e) => setVehicleForm({ ...vehicleForm, make: e.target.value })} />
          <input className="input-field w-auto" placeholder="Model" value={vehicleForm.model} onChange={(e) => setVehicleForm({ ...vehicleForm, model: e.target.value })} />
          <input
        ...
        <div className="mt-3 space-y-2">
          {loaded && vehicles.length === 0 && <p className="text-sm text-gray-500">No vehicles yet.</p>}
          {vehicles.map((vehicle) => (
            <div key={vehicle.id} className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-gray-100 px-4 py-3">
              <div>
                ...
                <p className="text-xs capitalize text-gray-500">{vehicle.service_type.replace("_", " ")}{vehicle.capacity ? ` · ${vehicle.capacity} seats` : ""}</p>
              </div>
      ...
      <div className="mt-6">
        <h3 className="text-sm font-bold uppercase tracking-wide text-gray-500">Drivers</h3>
        <form onSubmit={addDriver} className="mt-2 flex flex-wrap items-end gap-2">
          <input
            className="input-field w-auto"
          ...
          />
          <input className="input-field w-auto" placeholder="Phone" required value={driverForm.phone} onChange={(e) => setDriverForm({ ...driverForm, phone: e.target.value })} />
          <input
            className="input-field w-auto"
        ...
        <div className="mt-3 space-y-2">
          {loaded && drivers.length === 0 && <p className="text-sm text-gray-500">No drivers yet.</p>}
          {drivers.map((driver) => (
            <div key={driver.id} className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-gray-100 px-4 py-3">
              <div>
                <p className="text-sm font-semibold text-gray-800">{driver.full_name}</p>
