    ...
    const initialize = async () => {
      try {
        ...
        if (!params?.accessToken && !params?.refreshToken && !params?.code) {
          throw new Error("This password reset link is invalid or missing its recovery token.");
        }
        ...
        if (userError || !userData.user?.email) {
          throw new Error("Your recovery session could not be verified. Please request a new password reset link.");
        }
        ...
        if (active) {
          setMessage(error instanceof Error ? error.message : "Unable to initialize the password recovery flow.");
          setIsError(true);
          setLoading(false);
        }
      }
  ...
  return (
    <div className="min-h-screen bg-slate-50 px-4 py-12 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-md rounded-[28px] border border-slate-200 bg-white p-8 shadow-sm">
        ...
        <p className="mt-2 text-sm text-slate-600">Use the recovery link from your email to continue.</p>
        ...
        {!loading ? (
          <form onSubmit={handleSubmit} className="mt-6 space-y-4">
            <div>
              ...
              <input id="new-password" type="password" value={password} onChange={(event) => setPassword(event.target.value)} className="w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm outline-none transition focus:border-[#0A4D8C] focus:ring-4 focus:ring-[#0A4D8C]/10" autoComplete="new-password" />
            </div>
            <div>
              <label className="mb-1 block text-sm font-semibold text-slate-700" htmlFor="confirm-password">Confirm password</label>
              <input id="confirm-password" type="password" value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} className="w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm outline-none transition focus:border-[#0A4D8C] focus:ring-4 focus:ring-[#0A4D8C]/10" autoComplete="new-password" />
            </div>
            <button type="submit" disabled={submitting} className="w-full rounded-2xl bg-[#0A4D8C] px-4 py-3 text-sm font-semibold text-white transition hover:bg-[#083a6b] disabled:cursor-not-allowed disabled:opacity-60">
