...
export default function AdminApplicationsPage() {
  ...
  const renderStatusTag = (status: string) => {
    ...
    return <span className={`inline-flex rounded-full border px-3 py-1 text-xs font-semibold ${style}`}>{label}</span>;
  };
  ...
  const handleAction = async (id: string, action: "approve" | "reject") => {
    if (!confirm(action === "approve" ? "Approve this application and create the ambassador account?" : "Reject this application?")) {
      return;
    }
        ...
        {selectedApplicationDetails ? (
          <aside className="rounded-[24px] border border-slate-200 bg-white p-5 shadow-[0_18px_45px_-24px_rgba(10,77,140,0.35)] md:p-6">
            ...
            <div className="mt-5 flex flex-wrap gap-3">
              <button
                type="button"
                onClick={() => void handleAction(selectedApplicationDetails.id, "approve")}
              ...
              <button
                type="button"
                onClick={() => void handleAction(selectedApplicationDetails.id, "reject")}
                disabled={processingId === selectedApplicationDetails.id || selectedApplicationDetails.status !== "pending"}
