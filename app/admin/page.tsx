    ...
    <div className="space-y-3">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div>
          ...
          <select
            ...
            className="w-full border border-gray-300 p-2.5 rounded-lg bg-white text-slate-900 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
          >
      ...
      <div className="flex gap-2 flex-wrap">
        <button
          onClick={save}
          disabled={!selected}
          className="bg-blue-950 hover:bg-blue-800 disabled:opacity-50 text-white px-4 py-2 rounded-lg font-semibold shadow-sm transition text-sm"
        >
          ✓ Set Fair
        </button>
        <button
          onClick={addNew}
          className="bg-white hover:bg-slate-50 border border-slate-200 text-slate-800 px-4 py-2 rounded-lg font-semibold shadow-sm transition text-sm"
        >
          ＋ Add route
        </button>
      </div>
...
const STATUS_COLORS: Record<string, { badge: string; button: string }> = {
  ...
  Confirmed: { badge: "bg-[#006B3F]/10 text-[#006B3F] border-[#006B3F]/30", button: "bg-[#006B3F] hover:bg-green-800" },
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="min-h-screen flex flex-col lg:flex-row">
        <aside className="w-full lg:w-72 bg-blue-950 text-white p-4 lg:p-6 shrink-0">
            ...
            {TABS.map((tab) => (
              <button
                ...
                className={`shrink-0 px-3 py-2 rounded-lg text-xs font-semibold transition ${
                  activeTab === tab.key
                    ? "bg-blue-900/60 border border-blue-200/30"
            ...
            {TABS.map((tab) => (
              <button
                ...
                className={`w-full text-left px-3 py-2.5 rounded-lg transition ${
                  activeTab === tab.key
                    ? "bg-blue-900/60 border border-blue-200/20 font-semibold"
                    : "opacity-70 hover:opacity-100 hover:bg-blue-900/30"
                }`}
              >
                {tab.icon} {tab.label}
              </button>
            ...
            <div className="flex gap-2 items-center">
              <div className="relative w-full sm:w-64">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-3 py-2 border border-slate-300 rounded-xl text-sm bg-white text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-4 focus:ring-blue-500/50 focus:border-blue-600"
                />
                {search && (
                  <button
                    onClick={() => setSearch("")}
            ...
            <>
                ...
                <div className="space-y-6">
                  <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-6 gap-3 sm:gap-4">
                    <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-100">
                      ...
                      <h3 className="text-2xl font-extrabold text-blue-950">{overviewStats.total}</h3>
                    </div>
                    <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-100">
                      <p className="text-xs text-slate-500">Active Trips</p>
                      <h3 className="text-2xl font-extrabold text-blue-950">{overviewStats.uniqueTrips}</h3>
                    </div>
                    <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-100">
                    ...
                    <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-100">
                      ...
                      <h3 className="text-2xl font-extrabold text-blue-950">{overviewStats.uniqueStudents}</h3>
                    </div>
                    <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-100">
                      <p className="text-xs text-slate-500">Total Seats</p>
                      <h3 className="text-2xl font-extrabold text-blue-950">{overviewStats.totalSeats}</h3>
                    </div>
                    <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-100">
                      <p className="text-xs text-slate-500">Total Revenue</p>
                      <h3 className="text-2xl font-extrabold text-[#006B3F]">MWK {overviewStats.totalRevenue.toLocaleString()}</h3>
                    </div>
                  </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-6">
                      {Object.entries(tripGroups).map(([tripId, passengers]) => {
                        ...
                        return (
                          <div
                            key={tripId}
                            className="bg-white rounded-2xl shadow-sm border border-slate-100 p-4 sm:p-5 hover:shadow-lg hover:border-blue-200 transition"
                          >
                            <div className="flex justify-between items-start mb-3 gap-2">
                              <div className="min-w-0 flex-1">
                                <h3 className="font-extrabold text-blue-950 text-sm sm:text-base truncate break-words-force">{tripId}</h3>
                            ...
                            <details className="text-xs">
                              <summary className="text-blue-600 hover:text-blue-800 cursor-pointer font-semibold">
                    ...
                    <div className="overflow-x-auto">
                      <table className="w-full text-sm">
                        ...
                        <tbody className="divide-y divide-slate-100">
                          {filtered.map((b, i) => (
                            <tr key={`${b.bookingId || i}`} className="hover:bg-slate-50 transition">
                ...
                <div className="space-y-4">
                  {studentGroups.length === 0 ? (
                    ...
                    studentGroups.map((student) => (
                      <div key={student.studentId} className="bg-white rounded-xl shadow-sm border border-slate-100 p-4 sm:p-5 hover:shadow-md transition">
                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                          <div className="min-w-0 flex-1">
                            <h3 className="font-bold text-blue-950 break-words-force">{student.name}</h3>
                            <p className="text-xs text-slate-500">ID: {student.studentId}</p>
                            <p className="text-xs text-slate-400">📱 {student.phone}</p>
                          </div>
                          <div className="flex gap-4 text-sm shrink-0">
                            <div className="text-center">
                              <p className="font-bold text-blue-950">{student.bookings.length}</p>
                              <p className="text-[10px] text-slate-500">Bookings</p>
                            </div>
                            <div className="text-center">
                              <p className="font-bold text-blue-950">{student.totalSeats}</p>
                              <p className="text-[10px] text-slate-500">Seats</p>
                            </div>
                            <div className="text-center">
                              <p className="font-bold text-[#006B3F]">MWK {student.totalSpent.toLocaleString()}</p>
                        ...
                        <details className="mt-3">
                          <summary className="text-xs text-blue-600 hover:text-blue-800 cursor-pointer font-semibold">
              ...
              {activeTab === "whatsapp" && (
                <div className="space-y-6">
                  <div className="bg-white border border-slate-100 rounded-xl shadow-sm p-6">
                    <h3 className="font-bold text-blue-950 mb-2">📢 WhatsApp Broadcast Studio</h3>
                  ...
                  {whatsappMsg && (
                    <div className="bg-white border border-slate-100 rounded-xl shadow-sm p-6">
                      <div className="flex justify-between items-center mb-3">
                        <h4 className="font-bold text-blue-950 text-sm">Your Broadcast Message</h4>
                        <button
                          onClick={copyWhatsAppMessage}
                          className="bg-blue-950 hover:bg-blue-800 text-white px-4 py-1.5 rounded-lg text-xs font-semibold transition"
                        >
                      ...
                      <textarea
                        ...
                        value={whatsappMsg}
                        rows={20}
                        className="w-full border border-slate-200 rounded-xl p-4 text-sm bg-white text-slate-900 font-mono focus:outline-none focus:ring-2 focus:ring-blue-500 resize-y whitespace-pre-wrap break-words-force"
                      />
                    </div>
                  )}
              ...
              {activeTab === "settings" && (
                <div className="max-w-2xl space-y-6">
                  <div className="bg-white border border-slate-100 rounded-xl shadow-sm p-6">
                    <h3 className="font-bold text-blue-950 mb-2">⚙️ System Configuration</h3>
                    <p className="text-sm text-slate-500 mb-4">Adjust ticket parameters, routes, and vehicle settings.</p>

                    {settingsMsg && (
                      <div className="bg-blue-50 border border-blue-200 text-blue-800 p-3 rounded-lg mb-4 text-sm">{settingsMsg}</div>
                    )}
                    ...
                    <div className="space-y-4">
                      ...
                      <div>
                        ...
                        {/*
                          ...
                          rows={6}
                          placeholder="Example:\nMzuzu → Lilongwe: 5000\nMzuzu → Blantyre: 8000"
                          className="w-full border border-gray-300 p-2.5 rounded-lg bg-white text-slate-900 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 resize-y"
                        />
                      ...
                      <button
                        ...
                        className="bg-blue-950 hover:bg-blue-800 text-white px-5 py-2.5 rounded-lg font-semibold shadow-sm transition"
                      >
                        💾 Save Settings
                      </button>
                    </div>
                  ...
                  <div className="bg-white border border-slate-100 rounded-xl shadow-sm p-6">
                    <h3 className="font-bold text-blue-950 mb-2">🔑 Change Admin Password</h3>
