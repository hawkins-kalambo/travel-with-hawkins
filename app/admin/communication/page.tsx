      ...
      <div className="rounded-[24px] border border-slate-200 bg-white p-5 shadow-[0_18px_45px_-24px_rgba(10,77,140,0.35)] md:p-6">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <h1 className="mt-2 text-3xl font-black text-slate-900">Enterprise communication workspace</h1>
            <p className="mt-2 max-w-2xl text-sm text-slate-600">
              Manage notifications, conversations, announcements, and support operations from a single command center.
            </p>
          </div>
          ...
          {[
            ...
            <button
              ...
              className={`rounded-lg px-4 py-2 text-sm font-semibold transition ${
                activeTab === tab.key ? "bg-[#0a4d8c] text-white" : "text-slate-700 hover:bg-slate-50"
              }`}
            >
              {tab.label}
            </button>
        ...
        <div className="p-6">
                  ...
                  <div className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
                    <section className="rounded-[24px] border border-slate-200 bg-white p-5 shadow-sm">
                      <div className="mb-4 flex items-center justify-between">
                        <h2 className="text-lg font-semibold text-slate-900">Recent communication activity</h2>
                    ...
                    <section className="rounded-[24px] border border-slate-200 bg-white p-5 shadow-sm">
                      <h2 className="text-lg font-semibold text-slate-900">Communication health</h2>
                      <div className="mt-4 space-y-3">
                        <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                          <p className="text-sm text-slate-500">Unread notifications</p>
                          <p className="mt-1 text-2xl font-black text-slate-900">{metrics.unreadNotifications}</p>
                        </div>
                        <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                          <p className="text-sm text-slate-500">Active conversations</p>
                          <p className="mt-1 text-2xl font-black text-slate-900">{metrics.activeConversations}</p>
                        </div>
                        <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                          <p className="text-sm text-slate-500">Broadcast readiness</p>
                          <p className="mt-1 text-2xl font-black text-slate-900">{metrics.broadcastCampaigns}</p>
                        </div>
                      </div>
                    </section>
                  </div>
          ...
          {activeTab === "notifications" && (
            <div className="space-y-4">
              <div className="flex flex-col gap-3 rounded-[24px] border border-slate-200 bg-slate-50 p-4 md:flex-row md:items-center">
                <input
                ...
                <button onClick={() => void handleMarkAllRead()} className="rounded-2xl border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-100">
                  Mark all read
                </button>
              </div>
              ...
              {filteredNotifications.length === 0 ? (
                ...
                <div className="space-y-3">
                  {filteredNotifications.map((notification) => {
                    ...
                    return (
                      <div key={notification.id} className={`rounded-2xl border p-4 ${isUnread ? "border-[#0a4d8c]/20 bg-[#f6fbff]" : "border-slate-200 bg-white"}`}>
                        <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                          <div className="flex-1">
                            <div className="flex flex-wrap items-center gap-2">
                              ...
                              <span className={`rounded-full border px-2 py-0.5 text-[11px] font-semibold uppercase tracking-[0.2em] ${getPriorityStyles(notification.priority)}`}>
                          ...
                          <div className="flex flex-wrap gap-2">
                            {isUnread ? (
                              <button onClick={() => void handleNotificationAction(notification.id, "read")} className="rounded-full bg-[#0a4d8c] px-3 py-1.5 text-xs font-semibold uppercase tracking-[0.2em] text-white hover:bg-[#083a6b]">
                                Mark read
                              </button>
                            ) : (
                              <button onClick={() => void handleNotificationAction(notification.id, "unread")} className="rounded-full border border-slate-200 bg-white px-3 py-1.5 text-xs font-semibold uppercase tracking-[0.2em] text-slate-700 hover:bg-slate-50">
                                Mark unread
                              </button>
                            )}
                            <button onClick={() => void handleNotificationAction(notification.id, "archive")} className="rounded-full border border-slate-200 bg-white px-3 py-1.5 text-xs font-semibold uppercase tracking-[0.2em] text-slate-700 hover:bg-slate-50">
                              Archive
                            </button>
                          </div>
                        </div>
