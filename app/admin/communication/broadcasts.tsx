...
export default function BroadcastCenterSection() {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setPinned(true);
      setMessage("Broadcast published successfully");
      await load();
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Unable to publish broadcast");
    } finally {
      setSending(false);
    }
  };
    ...
    <div className="space-y-6">
      <div className="rounded-[24px] border border-slate-200 bg-white p-5 shadow-sm">
        <div className="mb-4 flex items-center justify-between">
          <div>
            <h3 className="text-lg font-semibold text-slate-900">Broadcast center</h3>
            <p className="mt-1 text-sm text-slate-600">Compose operational updates for ambassadors, admins, or everyone.</p>
          </div>
        </div>

        <div className="grid gap-4 lg:grid-cols-[0.8fr_1.2fr]">
          <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
            <h4 className="text-sm font-semibold uppercase tracking-[0.2em] text-slate-500">Templates</h4>
            <div className="mt-3 space-y-2">
              {broadcastTemplates.map((template) => (
                <button
                  key={template.label}
                  onClick={() => applyTemplate(template)}
                  className="flex w-full items-center justify-between rounded-xl border border-slate-200 bg-white px-3 py-2 text-left text-sm font-semibold text-slate-700 hover:bg-slate-50"
                >
                  <span>{template.label}</span>
                  <span className="text-xs uppercase tracking-[0.2em] text-slate-500">Use</span>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            {message ? (
              <div className={`rounded-lg p-3 text-sm ${message.includes("success") ? "bg-emerald-50 text-emerald-700" : "bg-slate-50 text-slate-700"}`}>
                {message}
              </div>
            ) : null}
      ...
      <div className="rounded-[24px] border border-slate-200 bg-white p-5 shadow-sm">
        ...
        {loading ? (
          <div className="text-sm text-slate-500">Loading recent broadcasts…</div>
              ...
              <li key={announcement.id} className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  ...
                  <div className="text-right text-xs text-slate-500">
                    ...
                    {announcement.pinned ? <div className="mt-1 text-amber-700">Pinned</div> : null}
                  </div>
                </div>
              </li>
            ))}
