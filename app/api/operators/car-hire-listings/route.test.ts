...
function makeGetRequest() {
  return new Request("https://example.com/api/operators/car-hire-listings") as unknown as Parameters<typeof GET>[0];
}

function makePostRequest(body: Record<string, unknown>) {
  return new Request("https://example.com/api/operators/car-hire-listings", {
    method: "POST",
...
test("POST returns 403 when the caller's staff role lacks manageRoutes", async () => {
  ...
  const res = await POST(makePostRequest({ vehicleId: "veh-1", dailyRate: 20000 }));
