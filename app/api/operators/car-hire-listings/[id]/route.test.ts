...
function makeRequest(method: string, body?: Record<string, unknown>) {
  return new Request("https://example.com/api/operators/car-hire-listings/listing-1", {
    method,
...
function makeParams() {
  return { params: Promise.resolve({ id: "listing-1" }) };
}
...
test("PATCH returns 404 (not found) when the car hire listing belongs to a different operator", async () => {
...
test("DELETE returns 404 (not found) when the car hire listing belongs to a different operator", async () => {
  authResult = { authorized: true, operatorId: "operator-A", staffRole: "owner", user: { id: "u-1" } };
  fromImpl = () => makeNoMatchBuilder();
  const res = await DELETE(makeRequest("DELETE"), makeParams());
  assert.equal(res.status, 404);
