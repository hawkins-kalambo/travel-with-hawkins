...
export async function PATCH(request: NextRequest) {
  ...
  // A suspended/rejected taxi or car_hire approval must stop new bookings
  // immediately — neither taxi_fares nor car_hire_listings has a direct
  // link back to service_approvals for the customer-facing browse queries
  // to check, so pull the rug here instead of requiring the operator to
  // remember to deactivate every listing themselves.
  if (status === "suspended" || status === "rejected") {
    const table = updated.service_type === "taxi" ? "taxi_fares" : updated.service_type === "car_hire" ? "car_hire_listings" : null;
    if (table) {
      const { error: deactivateError } = await supabaseAdmin
        ...
        .eq("status", "active");
      if (deactivateError) console.error(`Failed to deactivate ${table} after service approval change`, deactivateError);
    }
  }
