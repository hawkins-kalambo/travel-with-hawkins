  ...
  return (
    <section id="help-center" className="scroll-mt-24 border-t border-slate-100 px-4 pb-12 pt-10 sm:border-t-0 sm:pt-0">
      <div className="mx-auto max-w-6xl">
        <p className="text-xs font-black uppercase tracking-[0.18em] text-orange">Frequently Asked Questions</p>
        <h2 className="text-2xl font-black text-navy sm:text-3xl">Got Questions? We&apos;ve Got Answers</h2>
        <div className="mt-5 grid gap-3 md:grid-cols-2">
          {FAQ_ITEMS.map((item) => (
            <details key={item.q} className="group rounded-xl border border-border-light bg-white px-5 py-3">
              <summary className="flex cursor-pointer list-none items-center justify-between gap-3 font-bold text-slate-900">
                {item.q}
                <IconChevronDown className="h-4 w-4 shrink-0 text-orange transition group-open:rotate-180" />
              </summary>
        ...
        <p className="mt-6 text-sm font-semibold text-slate-600">
          Still need help? <a href="#contact" className="font-black text-orange hover:underline">Contact us</a>
        </p>
      </div>
    </section>
  );
